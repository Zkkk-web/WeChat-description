export function readConfig(env = process.env) {
  return {
    ...
    wechatArticles: {
      ...
      subscriptionStatePath: env.WECHAT_SUBSCRIPTION_STATE_PATH ?? ".codex-tmp/wechat-subscriptions.json",
      ...
      agentApiToken: env.WECHAT_AGENT_API_TOKEN ?? "",
      ...
      aiSummary: {
        enabled: env.WECHAT_ARTICLE_AI_SUMMARY_ENABLED === "true",
        apiBase: (env.WECHAT_ARTICLE_AI_BASE_URL ?? "").replace(/\/+$/, ""),
        apiKey: env.WECHAT_ARTICLE_AI_API_KEY ?? "",
        model: env.WECHAT_ARTICLE_AI_MODEL ?? "",
        timeoutMs: Number.parseInt(env.WECHAT_ARTICLE_AI_TIMEOUT_MS ?? "30000", 10),
        maxInputChars: Number.parseInt(env.WECHAT_ARTICLE_AI_MAX_INPUT_CHARS ?? "6000", 10),
      },
      ...
      useLocalSubscriptions: env.WECHAT_ARTICLE_LOCAL_SUBSCRIPTIONS !== "false",
      allowUpstreamSubscriptionFallback: env.WECHAT_ARTICLE_UPSTREAM_SUBSCRIPTION_FALLBACK === "true",
    },
  };
}
